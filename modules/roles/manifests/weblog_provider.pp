class roles::weblog_provider {
	if ! $::weblogsync_key {
		...
		file { '/etc/cron.d/puppet-weblog-provider':
			content => "SHELL=/bin/bash\n\n0 */4 * * * weblogsync sleep $((RANDOM \% 1800)); rsync -a --delete-excluded --include 'www.debian.org-access.log-*gz' --exclude '**' /var/log/apache2/. weblogsync@wolkenstein.debian.org:-weblogs-incoming-\n",
		}
	}
}
